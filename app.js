...
function renderHome(){
  ...
  document.getElementById('home-budget-pct').textContent=lvlPct+'%';
  document.getElementById('home-level-name').textContent='Level '+(lvl+1)+' · '+lv.name;
  if(state.goals.length>0){
    var g=state.goals[0];
    var gpct=Math.min(Math.round(state.sparGuthaben/g.target*100),100);
    document.getElementById('home-goal-pct').textContent=gpct+'%';
    document.getElementById('home-goal-name').textContent=g.emoji+' '+g.name;
  } else {
    document.getElementById('home-goal-pct').textContent='–';
    document.getElementById('home-goal-name').textContent='Kein Ziel gesetzt';
  }
...
function renderLearn(){
  ...
  KAPITEL.forEach(function(k,i){
    ...
    var isNew=i===done;var isLocked=i>done;
  ...
  state.goals.forEach(function(g,i){
    ...
    html+='<div class="goal-item">' +
      '<div class="goal-header">' +
        ...
        '<div class="goal-pct">'+(pct>=100?'<span style="color:var(--ok)">Erreicht!</span>':pct+'%')+'</div>' +
        '<button onclick="delGoal('+i+')" style="background:none;border:none;font-size:16px;opacity:.4;cursor:pointer;margin-left:6px" aria-label="Löschen">🗑</button>' +
      '</div>' +
...
function selectGoalIcon(emoji,el){
  selectedIcon=emoji;
  document.querySelectorAll('.icon-pick-btn').forEach(function(b){b.style.background='#F5F5F5';b.style.border='1.5px solid transparent';});
  el.style.background='#FFF0F1';el.style.border='1.5px solid #E30613';
}
...
function openGoalModal(){
  ...
  setTimeout(function(){
    var btns=document.querySelectorAll('.icon-pick-btn');
    btns.forEach(function(b,i){
      if(i===0){b.style.background='#FFF0F1';b.style.border='1.5px solid #E30613';}
      else{b.style.background='#F5F5F5';b.style.border='1.5px solid transparent';}
    });
  },50);
}
...
function saveGoal(){
  ...
  if(!n||!a||a<=0){showToast('Name und Betrag eingeben');return;}
  ...
  earnBadge('first-goal');
  if(state.goals.length>=3)earnBadge('visionary');
  if(a>=1000)earnBadge('bigdream');
  addXP(10);save();renderGoals();renderHome();showToast('Sparziel erstellt! +10 XP');
}
...
function renderHero(){
  ...
  document.getElementById('hero-streak-days').textContent=state.streak;
  document.getElementById('hero-level-name').textContent=lv.name;
  document.getElementById('hero-xp').textContent=state.xp;
  document.getElementById('hero-level').textContent=lvl+1;
  ...
  if(!state.quizAnswered){
    ...
    document.getElementById('hero-quiz-container').innerHTML=qh+'</div>';
  }
  var bh='';
  BADGES.forEach(function(b){
    var e=state.badges.indexOf(b.id)>-1||badgeAuto(b.id);
...
function answerQuiz(el,chosen,correct,xpVal){
  ...
  if(chosen===correct){addXP(xpVal);showToast('Richtig! +'+xpVal+' XP');}else showToast('Nicht ganz — richtige Antwort ist markiert');
  save();
}

function badgeAuto(id){
  ...
  if(id==='goal-done')return state.goals.some(function(g){return state.sparGuthaben>=g.target;});
  return false;
}
...
function load(){
  try{
    var s=localStorage.getItem('blkb_v2');
    if(s){
      var p=JSON.parse(s);
      var haxxIds=HAXX.map(function(h){return h.id;});
      if(p.haxxDone&&!p.badges){p.badges=p.haxxDone.filter(function(id){return haxxIds.indexOf(id)===-1;});p.haxxDone=p.haxxDone.filter(function(id){return haxxIds.indexOf(id)>-1;});}
